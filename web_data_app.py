...
def book_data():
    ...
    soup = BeautifulSoup(
        source.content, "html.parser"
    )  # Use BeautifulSoup to parse the website html code
    book_results = soup.find_all(
        attrs={"class": "product_pod"}
    )  # By inspecting the site, we know books = product_pod class
    ...
    book_data = pd.DataFrame(
        list(zip(titles, prices, sale_prices)),
        columns=["Titles", "Prices", "Sale_Prices"],
    )
    ...
    return render_template(
        "template.html",
        tables=[book_data.to_html(classes="data")],
...
def learn():
    ...
    return "I learned how to use Dataframes, Series, and Index objects in Pandas."
